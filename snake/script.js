...
const SCR = document.getElementsByClassName('value');
...
const game = {
    ...
    stop() {
        ...
        //Написать функцию выхода из игры.
        let stoped = document.getElementById('button-stop');
        function stp(event){
        let target = event.target.id;

            if (target === 'button-stop') {
            return alert('Игра окнчена ! ' + 'Ваш счет :' + ` ${SCR[0].innerHTML}`),
            board.clear(),
            snake.clearscore(),
            game.setGameStatus(GAME_STATUS_GRAY);
            }
            
        
        };
        stoped.addEventListener('clik', stp(event));
        //если голова пересикается с телом то game.stop
        /* добавить сюда код */
    },
    ...
    move(event) {
        ...
        if (event.keyCode = this.case) {
            this.case.addEventListener('keyup',setInterval(snake.setDirection(direction),300))
...
//let interval = setInterval(game.move(event), 300);
...
clear(){
        const clearElem = document.getElementsByClassName('board');
        clearElem[0].innerHTML = ' ';
    },
...
const snake = {
    ...
    setPosition(position, shift = true) {
        ...
        const scoreLengs = this.parts.length - 3;

        DIV_SCORE.innerHTML = scoreLengs;

        SCR[0].innerHTML = DIV_SCORE.innerHTML;


        //циклом проходимся по массиву тела и сравниваем есть ли такие коордиинаты.
        //сломал голову над этим. Буду рад помощи.

        for (let i = 0; i < this.parts.length; i++) {
            console.log(this.parts[i]);
            if (this.parts[0].top === this.parts[i].top && this.parts[0].left === this.parts[i].left) {
                console.log('hjugifuhgfui')
            }
